...
public class RestApiTestContext {    
    private final static Logger LOGGER = LogManager.getLogger(RestApiTestContext.class);    
    
    @Bean(name="sparqlRepository", initMethod = "initialize", destroyMethod = "shutDown")
    public Repository sparqlRepository( Environment env) throws RepositoryException { 
        // For test we use only in memory            
